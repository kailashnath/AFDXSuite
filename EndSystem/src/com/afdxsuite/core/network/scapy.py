...
#tof temp solution (dest can be a list)
...
#Function modified by tof so object can be pickled
class RandNum(RandField):
    min = 0
    max = 0
    def __init__(self, min, max):
        ## tof modif so that object can be pickled (generator cannot be pickled)
        # but unicity no guaranty
        self.min = min
        self.max = max
        # - tof
...
lfilter: truth function to apply to each packet to decide whether it will be displayed"""
    ...
    def _dump_document(self, **kargs):
        ...
        for i in range(len(self.res)):
            ...
            cbb = c.bbox()
            c.text(cbb.left(),cbb.top()+1,r"\font\cmssfont=cmss10\cmssfont{Frame %i/%i}" % (i,l),[pyx.text.size.normal])
            if conf.verb >= 2:
                os.write(1,".")
...
#                loctrace.append((ip,loc)) # no labels yet
        ...
        tr = map(lambda x: Gnuplot.Data(x,with_lines="lines"), trt.values())
        g = Gnuplot.Gnuplot()
        world = Gnuplot.File(conf.gnuplot_world,with_lines="lines")
        g.plot(world,*tr)
        return g
...
#        backcolor=makecol(0.376, 0.729, 0.525, 1.0)
        ...
        while t:
            ...
            proto,fields = t.pop()
            y += 0.5
            pt = pyx.text.text(XSTART, (YTXT-y)*YMUL, r"\font\cmssfont=cmss10\cmssfont{%s}" % proto.name, [ pyx.text.size.normal])
            y += 1
            ptbb=pt.bbox()
...
#Better version but not pickable
class ICMP2(Packet):
    name = "ICMP"
    fields_desc = [ ByteEnumField("type",8, icmptypes),
                    ByteField("code",0),
...
class L3dnetSocket(SuperSocket):
    ...
    def send(self, x):
        if isinstance(x, IPv6):
            ...
            #l_dst=x.dst
            #if type(l_dst) is list:
            #    l_dst=l_dst[0]
            #iff,a,gw = conf.route.route(l_dst)
            iff,a,gw = conf.route.route(x.dst)
        else:
            iff = conf.iface
...
### sendpfast for tcpreplay version >=3
def sendpfast3(x, pps=None, mbps=None, realtime=None, loop=0, multiplier=None, iface=None, fullspeed=False):
...
### sendpfast for tcpreplay version 2.3.x 
def sendpfast2(x, pps=None, mbps=None, realtime=None, loop=0, multiplier=None, iface=None, fullspeed=False):
    """Send packets at layer 2 using tcpreplay for performance
    pps:  packets per second
    mpbs: MBits per second
    realtime: use packet's timestamp, bending time with realtime value
    ...
    if iface is None:
        iface = conf.iface
    args = [conf.prog.tcpreplay, '-i', iface]
    if pps is not None:
        args+=['-p','%i'%pps]
    elif mbps is not None:
        args+=['-r','%i'%mbps]
    ...
    try:
        try:
            r=os.spawnlp(os.P_WAIT, conf.prog.tcpreplay, *args)
            print 'spawnlp return '+repr(r)
        except KeyboardInterrupt:
            log_interactive.info("Interrupted by user")
    finally:
        os.unlink(f)
...
### Set default sendpfast function to 2.3x version
sendpfast=sendpfast2
...
class Automaton_metaclass(type):
    ...
    def graph(self, **kargs):
        ...
        sb = "" # Keep initial node at the begining for better rendering
        sm = "" # Keep middle nodes in the middle
        se = "" # Keep last node at the end 
        
        for st in self.states.itervalues():
            if st.atmt_initial:
                sb += '\t"%s" [fontname="Courier", fontsize=10, style=filled, fillcolor=blue, shape=box, root=true];\n' % st.atmt_state
            elif st.atmt_final:
                se += '\t"%s" [fontname="Courier", fontsize=10, style=filled, fillcolor=green, shape=octagon ];\n' % st.atmt_state
            elif st.atmt_error:
                se += '\t"%s" [fontname="Courier", fontsize=10, style=filled, fillcolor=red, shape=octagon ];\n' % st.atmt_state
            else:
                sm+= '\t"%s" [fontname="Courier", fontsize=10];\n' % st.atmt_state
        s += sb + sm + se
                ...
                for n in f.func_code.co_names+f.func_code.co_consts:
                    if n in self.states:
                        ...
                        for x in self.actions[f.atmt_condname]:
                            l += "\\l>[%s]" % x.func_name
                        s += '\t"%s" -> "%s" [fontname="Courier", fontsize=10, label="%s", color=%s];\n' % (k,n,l,c)
            ...
            for t,f in v:
                ...
                for n in f.func_code.co_names+f.func_code.co_consts:
                    if n in self.states:
                        ...
                        for x in self.actions[f.atmt_condname]:
                            l += "\\l>[%s]" % x.func_name
                        s += '\t"%s" -> "%s" [fontname="Courier", fontsize=10, label="%s",color=blue];\n' % (k,n,l)
        s += "}\n"
        open('mongraph.txt','aw').write(s)
